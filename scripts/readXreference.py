    ...
    for r in refInf.ref:
        ...
        if (r.kind == 0 or r.kind == 1):
            ...
            if directFlowInst(disassemble_content, inst_addr):
                ...
                logging.debug("Should skip the direct jump/call instruction 0x%x." % ref_addr)
                continue
        result[ref_addr] = target_addr
        logging.debug("The reference instruction address is 0x%x" % (ref_addr))
        logging.debug("The reference target address is 0x%x" % (target_addr))
